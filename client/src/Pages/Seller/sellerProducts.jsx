...
export default function SellerProducts() {
  ...
  async function loadProducts() {
    try {
      ...
      setProducts(res.products || []); // backend: { success, products } or similar
    } catch (err) {
      console.error("Load products error:", err);
      alert(err?.message || err?.data?.message || "Failed to load products");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {products.map((p) => (
            <div key={p._id} className="p-4 bg-white rounded shadow">
              <img src={p.image || "/assets/default-product.png"} alt={p.name} className="w-full h-44 object-cover rounded mb-3" />
